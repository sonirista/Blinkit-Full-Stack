  ...
  return (
    <section>
      <div className='p-2  bg-white shadow-md flex items-center justify-between '>
        ...
        <button
          className='text-sm border border-amber-300 hover:bg-amber-600 px-3 py-1 rounded '>Add Category</button>
      </div>
      <div className='grid p-3'>
        <form className='grid gap-4' onSubmit={handleSubmit} >
              ...
              Object?.keys(data?.more_details)?.map((k, index) => {
                return (
                  <div className='grid gap-1'>
                    ...
                    <input id={k} type='text'  value={data?.more_details[k]} onChange={(e)=>{const value = e.target.value 
                    setData((preve)=>{
                      return {
                        ...preve,
                        more_details : {
          ...
          <div onClick={() => setOpenAddField(true)} className='hover:bg-yellow-300 hover:bg-white  py-1 px-3 w-32 text-center font-semibold border border-yellow-300 hover:text-neutral-900 cursor-pointer rounded'>
            Add Fields
          </div>
           <button className='bg-yellow-300 hover:bg-yellow-400 py-2 rounded font-semibold'>Submit</button>
        </form>
      </div>
      {
        viewImageUrl && (
